#!/usr/bin/env python3
"""Call MinerU's official API, validate the result, and archive the usable draft."""
...
CODEX_EXTRACTION_RELATIVE = Path("04_资料与证据/20_原文提取/10_Codex提取")
...
def stage_markdown(selected: Path, stage_root: Path, destination_name: str) -> tuple[Path, Path | None, int]:
    ...
    asset_root = stage_root / f"{Path(destination_name).stem}_assets"
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        source_asset = (selected.parent / decoded).resolve()
        if not source_asset.is_file():
            raise MinerUError(f"Markdown引用的本地图片不存在：{raw_target}", "asset-check")
        if source_asset not in copied:
            base = sanitize_name(source_asset.stem) + source_asset.suffix.lower()
            target = asset_root / base
            counter = 2
            while target.exists():
                target = asset_root / f"{sanitize_name(source_asset.stem)}-{counter}{source_asset.suffix.lower()}"
                counter += 1
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source_asset, target)
            copied[source_asset] = f"{asset_root.name}/{target.name}"
        rendered = f"<{copied[source_asset]}>" if angle else copied[source_asset]
        return f"{match.group(1)}{rendered}{suffix}{match.group(3)}"

    rewritten = IMAGE_LINK_RE.sub(replace, text)
    destination.write_text(rewritten, encoding="utf-8", newline="\n")
    return destination, asset_root if copied else None, len(copied)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="通用MinerU API调用与Codex提取稿归档")
    ...
    extract = subparsers.add_parser("extract", help="通过MinerU API提取一份文件并归档为Codex提取稿")
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="mineru-api-wrapper-test-") as temp:
        ...
        ledger = project / "90_归档/20_项目运行历史/项目运行账本.jsonl"
        if not expected.is_file() or not ledger.is_file():
            return 1
        if "_assets/figure.png" not in expected.read_text(encoding="utf-8"):
            return 1
        ...
        if run_extract(failed_args, FakeApi(fail=True)) != 2:
            return 1
        failed_ledger = failed_project / "90_归档/20_项目运行历史/项目运行账本.jsonl"
