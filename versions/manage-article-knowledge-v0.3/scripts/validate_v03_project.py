...
def validate_project(root: Path, runtime_skill: Path | None = None) -> list[Issue]:
    ...
    if customer_request_file.is_file():
        ...
        if customer_table is None:
            ...
            if headers != list(CUSTOMER_REQUEST_HEADERS):
                add("ERROR", "CUSTOMER_REQUEST_TABLE", customer_request_file, 1, "待客户补充事项字段顺序或数量不符合v0.3固定结构")
    ...
    if material_todo_file.is_file():
        ...
        if material_table is None:
            ...
            if headers != list(MATERIAL_TODO_HEADERS):
                add("ERROR", "MATERIAL_TODO_TABLE", material_todo_file, 1, "源资料处理待办字段顺序或数量不符合v0.3固定结构")
    ...
    if skill_feedback_file.is_file():
        ...
        if feedback_table is None:
            ...
            if headers != list(SKILL_FEEDBACK_HEADERS):
                add("ERROR", "SKILL_FEEDBACK_TABLE", skill_feedback_file, 1, "Skill反馈表字段顺序或数量不符合v0.3固定结构")
        ...
        if not deposition_log.is_file():
            ...
            if event_headers != list(DEPOSITION_EVENT_HEADERS):
                add(
                    ...
                    deposition_log,
                    1,
                    "正式知识沉淀事件记录字段不符合v0.3固定结构，且不得包含文章使用字段",
                )
    ...
    if not candidate_file.is_file():
        ...
        if candidate_table is None:
            add("ERROR", "CANDIDATE_LEDGER_SCHEMA", candidate_file, 1, "知识候选分流表缺少v0.3固定字段")
        else:
            headers, rows = candidate_table
            if headers != list(CANDIDATE_HEADERS):
                add("ERROR", "CANDIDATE_LEDGER_SCHEMA", candidate_file, 1, "知识候选分流表字段顺序或数量不符合v0.3固定结构")
    ...
    if not deposition_state.is_file():
        ...
        if state_table is None or state_table[0] != list(DEPOSITION_STATE_HEADERS):
            add("ERROR", "DEPOSITION_STATE_SCHEMA", deposition_state, 1, "知识沉淀记录缺少v0.3固定字段")
    ...
    if not usage_detail.is_file():
        ...
        if usage_headers != list(CLAIM_USAGE_HEADERS):
            add("ERROR", "CLAIM_USAGE_SCHEMA", usage_detail, 1, "知识块使用明细字段不符合v0.3固定结构")
        else:
    ...
    if not usage_total.is_file():
        ...
        if total_table is None or total_table[0] != list(CLAIM_TOTAL_HEADERS):
            add("ERROR", "CLAIM_USAGE_TOTAL_SCHEMA", usage_total, 1, "知识块使用总表缺少v0.3固定字段")
        else:
...
def create_self_test_project(root: Path) -> None:
    ...
    (root / VERSION_ENTRY_RELATIVE).write_text(
        "# 版本与变更入口\n\n- 项目ID：TEST\n- 当前状态：[[20_当前待办]]\n- 使用Skill：manage-article-knowledge v0.3\n"
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="validate-v03-") as temp:
        ...
        valid_run_record = {
            ...
            "skill": {"name": "manage-article-knowledge", "version": "v0.3", "sha256": "test"},
            "inputs": [],
            "read_files": [],
            "tool_calls": [],
            "outputs": [],
