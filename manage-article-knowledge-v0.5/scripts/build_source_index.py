...
def extract_file(path: Path) -> tuple[list[tuple[str, str]], str, str, str]:
    ...
    try:
        if ext in TEXT_EXTS:
            ...
            indexed_scope = "deterministic text extraction"
        elif ext in HTML_EXTS:
            parser = TextHTMLParser()
            parser.feed(read_text(path))
            chunks = split_chunk("HTML visible text", "\n".join(parser.parts))
            indexed_scope = "visible HTML text"
            excluded_scope = "scripts, styles and interactive state"
        elif ext == ".docx":
            chunks, has_media = extract_docx(path)
            indexed_scope = "main-document paragraph and table text"
            excluded_scope = "headers, footers, comments and drawing text"
            if has_media:
                excluded_scope += "; embedded images and objects"
        elif ext == ".pptx":
            chunks, has_media = extract_pptx(path)
            indexed_scope = "slide text"
            excluded_scope = "speaker notes, masters and some chart structure"
            if has_media:
                excluded_scope += "; embedded images, charts and objects"
        elif ext == ".xlsx":
            chunks, has_media = extract_xlsx(path)
            indexed_scope = "cell values"
            excluded_scope = "formulas, comments, styling and chart structure"
            if has_media:
                excluded_scope += "; embedded images and charts"
        elif ext == ".pdf":
            chunks, note = extract_pdf(path)
            indexed_scope = "text-bearing PDF pages" if chunks else ""
            excluded_scope = "page images and non-extractable structure"
        elif ext in ARCHIVE_EXTS:
            chunks, note = extract_archive_listing(path)
            indexed_scope = "archive member names" if chunks else ""
            excluded_scope = "archive member contents"
        elif ext in AUDIO_EXTS or ext in VIDEO_EXTS:
            note = "仅基础信息，内容未检索"
            excluded_scope = "audio/video content"
        elif ext in IMAGE_EXTS:
            note = "image metadata only"
            excluded_scope = "visual content"
        elif ext in LEGACY_OFFICE_EXTS:
            note = "legacy Office format requires specialist-approved handling"
            excluded_scope = "document content"
        else:
            note = "unsupported format; metadata only"
            excluded_scope = "file content"
