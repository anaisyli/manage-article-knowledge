...
CONDITION_FIELDS = ("关闭条件", "重开条件")
NEGATIVE_VALIDATION_RE = re.compile(
...
def linked_targets(path: Path, text: str) -> list[str]:
    """Return relative, file-like Wikilinks that do not resolve from the ledger."""
    project_root = path.parents[4] if len(path.parents) > 4 else path.parent
    result: list[str] = []
...
def audit(path: Path) -> list[dict[str, object]]:
    ...
    for row in feedback_rows(text):
        ...
        result.append({
            ...
            "has_close_condition": bool(re.search(r"(?m)^-\s*关闭条件[：:]\s*\S", detail)),
            "has_reopen_condition": bool(re.search(r"(?m)^-\s*重开条件[：:]\s*\S", detail)),
            "has_close_evidence": bool(re.search(r"(?m)^-\s*关闭依据[：:]\s*\S", detail)),
            "validation_contradiction": bool(NEGATIVE_VALIDATION_RE.search(detail)),
            "broken_links": linked_targets(path, detail),
...
def transition(
    ...
    if target_stage == "已关闭":
        for field in CONDITION_FIELDS:
            if not re.search(rf"(?m)^-\s*{re.escape(field)}[：:]\s*\S", block):
                raise ValueError(f"关闭SKFB前详情必须填写{field}")
        if NEGATIVE_VALIDATION_RE.search(block):
            raise ValueError("关闭SKFB前详情仍包含未验证、待执行或失败的项目验证记录")
    ...
    if target_stage == "已关闭":
        verification.append(f"- 关闭依据：{evidence}")
