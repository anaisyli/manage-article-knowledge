...
ARTICLE_AUDIT_TEMPLATE_VERSION = "v0.5-20260826"
...
def validate_article_governance_checks(
    ...
    for path in task_root.rglob("20_文章前知识审核.md"):
        ...
        if not template_version:
            warnings.append(f"既有文章审核未使用大纲覆盖与CUS/ANM主动检测模板：{path}")
            continue
...
def run_self_test() -> int:
    with tempfile.TemporaryDirectory(prefix="v05-claim-check-test-") as temp:
        root = Path(temp)
        external = root / "03_正式知识/20_外部公共知识/选型原则.md"
        customer = root / "03_正式知识/10_客户知识/客户能力.md"
...
""",
    ...
    with tempfile.TemporaryDirectory(prefix="v05-governance-check-test-") as temp:
        root = Path(temp)
